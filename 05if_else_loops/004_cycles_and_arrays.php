...
//$arNum - счетчик элементов массива
$boringToys = [];
$arNum = rand(1, 10);

for ($item = 1; $item <= $arNum; $item++) {
 ...
 *
 */
$colors = ['блю', 'fiol', 'rose', 'red', 'white', 'seriy'];
...
foreach($cars as $key => $value) {
    foreach($cars[$key]['colors'] as $colorKey => $colorItem) {
        echo "Купите автомобиль " . $cars[$key]['name'] . " цвета " . $cars[$key]['colors'][$colorKey]['name'] . " всего за: " . ($cars[$key]['price'] + $cars[$key]['colors'][$colorKey]['xPrice']) . " руб. " .'<br>';
    }
